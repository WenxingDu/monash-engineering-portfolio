# prepDis = math.sqrt((robot.state[0][0] - targetX)**2 + (robot.state[1][0] - targetY) **2)
import cv2
import os
import numpy as np
...
class Detector:
    ...
    def _get_bounding_boxes(self, cv_img):
        ...
        predictions = self.model.predict(cv_img, imgsz=320, verbose=False, conf=0.6, iou=0.7)
        ...
        for prediction in predictions:
            ...
            for box in boxes:
                ...
                # print(box_label)
                # print(f"noint box is  \n\n\n {bounding_boxes}")
                bounding_boxes.append([self.model.names[int(box_label)], np.asarray(box_cord)])
...
if __name__ == '__main__':
    ...
    img = cv2.imread(f'{script_dir}/test/pred_6.png')
