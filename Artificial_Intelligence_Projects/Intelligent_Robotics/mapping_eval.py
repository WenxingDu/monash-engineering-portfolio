...
# python3 Week02-04/mapping_eval.py --true-map M3_GroundTruths/LabB_1.txt --slam-est OJ_SUBMIT/slam2.txt --target-est OJ_SUBMIT/targets2.txt
...
def align_object_poses(theta, x, objects_est):
    ...
    return objects
# python mapping_eval.py --slam-est lab_output/SUBMIT/slam_116.txt --target-est lab_output/SUBMIT/targets_116.txt
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--true-map', type=str, default='lab_output/MAP_13_OCT.txt')
    parser.add_argument('--true-map', type=str, default='lab_output/MAP_13_OCT.txt')
    parser.add_argument('--slam-est', type=str, default='lab_output/slam.txt')

    parser.add_argument('--target-est', type=str, default='lab_output/targetsUPSTAR.txt')
    # parser.add_argument('--target-est', type=str, default='lab_output/SUBMIT/targets_116.txt')
    ...
    if slam_only:
        ...
        print(f"assuming no collusion, the score for M2 is {((0.12 - slam_rmse_aligned)/(0.12 - 0.02)) * 80 +20}")
        ...
        print('======')
        print(f'Target Estimation Error Score (0 to 80): = {np.sum(target_scores)}, ({(100*np.sum(target_scores))/80}%)')
