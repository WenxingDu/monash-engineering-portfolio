...
def is_in_obstacle(node, obstacles):
    ...
    for (ox, oy, radius) in obstacles:
        if euclidean_distance((x, y), (ox, oy)) < radius:
            return True
    return False
...
def edge_intersects_obstacle(node1, node2, obstacles):
    for (ox, oy, radius) in obstacles:
        ...
        if euclidean_distance(closest_point, (ox, oy)) < radius:
            return True
    return False
...
def plan_path_Astar(start, goal, obstacles_given,endTime = 3):
    # Create graph
    G = nx.Graph()
...
def plot_single_path_and_obstacles(aStar,pos,path, obstacles, start, goal):
    """
...
if __name__ == "__main__":
    ...
    goalObject = obstacles_default.get('tomato_0')
    # input(test)
    xCoord = goalObject.get('x')
    yCoord = goalObject.get('y')       
    goal_point = (xCoord, yCoord)    
    # goal_point = (1.4, 1.4)    
    ...
    while findingPath:
        path,pos, G = plan_path_Astar(start_point, goal_point, obstacles_default)
        if path == None:
