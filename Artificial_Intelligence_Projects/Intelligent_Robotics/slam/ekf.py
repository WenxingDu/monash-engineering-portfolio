...
class EKF:
    ...
    def update(self, measurements):
        ...
        for i in range(len(measurements)):
            ...
            # if prepDist < 1.5:
            weight = 240*(prepDist-0.3)**2 #200 be good ,0.2
            # weight = 240*(prepDist-0.3)**2 #200 be good ,0.2
            # weight = 1
    ...
    def draw_slam_state_fixed(self, res = (300, 300), not_pause=True):
        # Draw landmarks
        m2pixel = 100
        # grid
        grid_spacing = 0.5 * m2pixel
        ...
        for x in range(-int(res[0]/2), int(res[0]/2), int(grid_spacing)):
            cv2.line(canvas, (int(res[0]/2) + x, 0), (int(res[0]/2) + x, res[1]), grid_colour, thickness=grid_thick)

        # Draw horizontal grid lines
        for y in range(-int(res[1]/2), int(res[1]/2), int(grid_spacing)):
            cv2.line(canvas, (0, int(res[1]/2) + y), (res[0], int(res[1]/2) + y), grid_colour, thickness=grid_thick)

        # border
        bord_size = 200
        bord_center = 0

        # Calculate top-left and bottom-right corners of the square
        top_left = (50, 50)
        bottom_right = (350, 350)

        # Draw the square on the canvas
        square_color = (255, 0, 0)  # Color of the square (green)
        cv2.rectangle(canvas, top_left, bottom_right, square_color, thickness=1)  # thickness=-1 fills the square

        # landmarks in meters, 
        lms_xy = self.markers[:2, :] # keep OG landmark positions
        robot_xy = self.robot.state[:2, 0].reshape((2, 1)) # keep actual robot position
        robot_theta = self.robot.state[2,0]

        # Convert robot position to pixel coordinates on the fixed map
        robot_pixel = self.to_im_coor(robot_xy.flatten(), res, m2pixel)
        
        # plot robot with its real coords on the map
        p_robot = self.P[0:2,0:2]
        axes_len,angle = self.make_ellipse(p_robot)
        canvas = cv2.ellipse(canvas, robot_pixel, 
                    (int(axes_len[0]*m2pixel), int(axes_len[1]*m2pixel)),
                    angle, 0, 360, (0, 30, 56), 1)
        
        # draw landmards in their real coordinates, relative to the fixed map
        if self.number_landmarks() > 0:
            for i in range(len(self.markers[0,:])):
                xy = (lms_xy[0, i], lms_xy[1, i])
                coor_ = self.to_im_coor(xy, res, m2pixel)
                
                # plot covariance for landmarks
        ...
        # draw the robot 
        surface.blit(self.rot_center(self.pibot_pic, robot_theta*57.3), (robot_pixel[0]-15, robot_pixel[1]-15))
        
        # draw landmarks as images at their fixed map coords
        if self.number_landmarks() > 0:
            for i in range(len(self.markers[0,:])):
                xy = (lms_xy[0, i], lms_xy[1, i])
                coor_ = self.to_im_coor(xy, res, m2pixel)
