...
class Robot:
    ...
    def drive(self, drive_meas):
        ...
        # print("\n\n======updating the state at drive")
        
        # print(f"dt is {dt}, new x is {self.state[0]}, new y is {self.state[1]}, new th is {self.state[2]}")
    ...
    def derivative_drive(self, drive_meas):
        ...
        # print(f"=========\n\nth is {th}\nang_vel is {ang_vel}\nlin_vel is {lin_vel}\n\n")
        if(ang_vel != 0):
            # DFx[0,2] = self.state[0] + (lin_vel / ang_vel) *(-np.cos(th) + np.cos(th + ang_vel *dt))
            # DFx[1,2] = self.state[1] + (lin_vel / ang_vel) *(-np.sin(th) + np.sin(th + ang_vel * dt))
        # else:
        #     DFx[0,2] = 0
        #     DFx[0,2] = 0
            
        # print(f"cos value is {np.cos(th + ang_vel *dt)}")
        # print(f" sin value is {np.sin(th + ang_vel * dt)}")
        
            DFx[0,2] = (lin_vel / ang_vel)* (-np.cos(th) + np.cos(th + ang_vel *dt))
            DFx[1,2] = (lin_vel / ang_vel)* (-np.sin(th) + np.sin(th + ang_vel * dt))
        
        # print(f"reslt for DFx is \n{DFx}\n")
        # print(f"value A is {(-np.cos(th) + np.cos(th + ang_vel *dt))}")
        # print(f"value B is {(-np.sin(th) + np.sin(th + ang_vel * dt))}")
        
        #TODO double check the self.state line, don't think we need taht 
    ...
    def covariance_drive(self, drive_meas):
        ...
        # dx/dV, dy/dV, dtheta/dV
        if(ang_vel != 0 ):
            Jac2[0,0] = (-np.sin(th)+np.sin(th2)) / ang_vel #dxdv
            Jac2[0,1] = ((lin_vel)/(ang_vel)) * ( (dt*np.cos(th2)) - ((np.sin(th2)- np.sin(th))/(ang_vel)) ) #dxdw
            
            
            Jac2[1,0] = (np.cos(th)-np.cos(th2)) / ang_vel #dydv
            Jac2[1,1] = lin_vel * ( -np.cos(th)/(ang_vel**2)     +      (dt * np.sin(th2))/ang_vel     -    np.cos(th2)/(ang_vel**2)  ) #dydw    
            
            Jac2[2,0] =  0 #dthdv
            Jac2[2,1] = dt #dthdw
        else:
            Jac2[0,0] = np.cos(th)#dxdv
            Jac2[0,1] =0 #dxdw
            
            Jac2[1,0] = np.sin(th) #dydv
            Jac2[1,1] = 0 #dydw
            Jac2[2,0] =  0 #dthdv
            Jac2[2,1] = 0#dthdw
