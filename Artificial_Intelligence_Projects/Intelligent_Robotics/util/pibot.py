...
class PenguinPi:
    ...
    ########################################## 
    def set_velocity(self, command, tick=100, turning_tick=20, time=0):
        # running in sim
        if self.ip == 'localhost': 
            l_vel = command[0]*tick - command[1]*turning_tick #if command 1 is 0, the car doens't turn as there's no offset
            r_vel = command[0]*tick + command[1]*turning_tick # setting the value with +- ve to add offset, allowing the car to turn 
    ...
    # get frame from simulated robot     ekf
